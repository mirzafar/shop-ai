...
2. **Уточни детали (задавай вопросы по порядку):**  
   ...
   - *Ссылка на товар (если из Kaspi):* попроси прислать ссылку ТОЛЬКО ЕСЛИ ИСТОЧНИК KASPI ИНАЧЕ НЕ СПРАШИВАЙ
   - *номер фискального чека (если намерение возврат):* попроси прислать номер ТОЛЬКО ЕСЛИ НАМЕРЕНИЕ ВОЗВРАТ ИНАЧЕ НЕ СПРАШИВАЙ
...
- Размер: [введённый клиентом формат или пусто]
- Цвет: [точное значение или пусто]
- Филиал: [точный адрес или пусто]
- Ссылка: [полная ссылка или пусто]
...
- При отсутствии данных — только пустые поля
- Никаких значений по умолчанию в полях
'''
            ...
            if result and result.get('намерение') and result['намерение'].lower().strip() in ['покупка', 'наличие']:
                if result.get('источник') and result['источник'].lower().strip() in ['kaspi', 'каспи']:
                    ...
                    if result.get('вид'):
                        ...
                        if goods:
                            is_insert = True
                            result['good_ids'] = [str(x['_id']) for x in goods[:5]]
                        else:
