...
async def func_sell(input_text: str | None, chat_id: str):
    system_message = '''
        ...
        – размер (если клиент не знает — предложи таблицу размеров европейские формате),
        – цвет.
    ...
    try:
        ...
        if flag is False:
            ...
            – вид одежды (например: платье, блузка, брюки и т.д.),
            – размер (если клиент не знает — предложи таблицу размеров, размер должен европейские формате),
            – цвет.
