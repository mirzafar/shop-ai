...
async def func_sell(input_text: str | None, chat_id: str):
    system_message = '''
        ...
        Диалог начни сразу с выбора одежды без приветствие, Закончива без (никаких прощаний, вопросов или предложений помощи), Не обсуждай другие темы, не предлагай ничего лишнего. Отвечай коротко, дружелюбно и по существу. Если не хватает одного или нескольких параметров — уточни.
        '''
    conversations = await cache.get(f'chatbot:{chat_id}:conversations')
    if conversations:
        conversations = ujson.loads(conversations)
